...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        # question에 달린 댓글이기에 question 값 넘겨준다.
...
def create_answer(answer_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        return redirect('{}#comment_{}'.format(url_for('question.detail', question_id=answer.id), comment.id))
    return render_template('comment/comment_form.html', form=form)
...
def modify_answer(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            return redirect('{}#comment_{}'.format(url_for('question.detail', question_id=comment.question.answer.id), comment.id))
    else:
        form = CommentForm(obj=comment)
    return render_template('comment/comment_form.html', form=form)
